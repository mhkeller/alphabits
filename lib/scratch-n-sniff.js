...
function scraper_fn(){
  ...
  scraper.format = function(value){
    if (!arguments.length) return format_type;
    format = value;
    return scraper
  }
  ...
  scraper.each = function(cb){
    ...
    data_obj.forEach(function(datum){
      opts = urlAccessor(datum)
      that.processRated(url_string + opts, datum, cb, true)
    })
  }
  ...
  scraper.process = function(url, datum, cb, async){
    ...
    request(url, function (err, response, body) {
      ...
      extractorFn(cheerio.load(body), opts, function(result){
        ...
        request_numb++
        if (request_numb == request_total || async){
          if (format == 'csv') { cb(dsv.csv.format(_.flatten(result_arr))); return false }
          cb(result_arr)
        }
